...
public class MarioLevel {
    ...
    public MarioLevel(String level, boolean visuals) {
        ...
        this.levelStr = new char[lines[0].length()][lines.length];
        ...
        for (int y = 0; y < lines.length; y++) {
            for (int x = 0; x < lines[y].length(); x++) {
                ...
                this.levelStr[x][y] = lines[y].charAt(x);
            }
        }
    ...
    public void setAStar(int x, int y) {
        if (x < 0 || y < 0 || x >= this.levelStr.length || y >= this.levelStr[0].length)
            return;
        this.levelStr[x][y] = 'A';
    }
    public String getLevelString() {
        ...
        for (int j = 0; j < this.levelStr[0].length; j++) {
            for (int i = 0; i < this.levelStr.length; i++) {
                char x = this.levelStr[i][j];
                s += x;
            }
            s += "\n";
        }
